...
class TestKylinOperator(unittest.TestCase):
    _config = {
    ...
    @patch('airflow.providers.apache.kylin.operators.kylin.KylinHook')
    def test_execute(self, mock_hook):
        operator = KylinOperator(
            task_id='kylin_task',
            dag=self.dag,
            **self._config
        )
        ...
        mock_hook.return_value.cube_run.assert_called_once_with(
            'kylin_sales_cube', 'build', end=datetime(2012, 1, 3, 0, 0),
            name=None, offset_end=None, offset_start=None, start=datetime(2012, 1, 2, 0, 0)
        )
